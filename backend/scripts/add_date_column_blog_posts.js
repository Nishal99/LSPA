...
(async () => {
  try {
    console.log('Adding `date` column to blog_posts (if not exists)...');
    await db.execute("ALTER TABLE blog_posts ADD COLUMN IF NOT EXISTS \`date\` DATE NULL DEFAULT NULL");
